...
{
    app_ir_feed_edge_isr((BOARD_IR_RX_READ() != 0u) ? 1u : 0u, board_timer0_read());
}
...
void main(void)
{
    ...
    while (1) {
        ...
        if ((app_light_is_on() == 0u) &&
            ...
            board_timer0_stop();
            entered_power_down = 0u;
            if ((IE0 == 0) && (BOARD_IR_RX_READ() != 0u)) {
                entered_power_down = 1u;
